...
describe('ApiKeyService', () => {
  ...
  it('rejects missing and unknown API keys during validation', async () => {
    ...
    jest.spyOn(service as any, 'findValidationRow').mockResolvedValue(null);
  ...
  it('rejects malformed API keys during validation', async () => {
    const { service } = createService();
    const findValidationRow = jest.spyOn(service as any, 'findValidationRow');
  ...
  it('rejects disabled API keys through EntityService lookup', async () => {
    ...
    jest.spyOn(service, 'findOne').mockResolvedValue(apiKey);
  ...
  it('validates valid API keys without touching last used time', async () => {
    ...
    jest
      .spyOn(service as any, 'findValidationRow')
      .mockResolvedValue(validationRow({ apiKey, workspace, workspaceMember }));
  ...
  it('validates selected workspace API keys without direct knex access', async () => {
    ...
    jest
      .spyOn(service as any, 'findValidationRow')
      .mockResolvedValue(validationRow({ apiKey, workspaceMember }));
...
function createService(
  overrides: Partial<jest.Mocked<EntityManager>> & {
  ...
  const {
    decryptedSecret = 'abcdef0123456789',
  ...
  const cryptService = {
    encrypt: jest.fn(async () => 'encrypted-secret'),
    decrypt: jest.fn(async () => decryptedSecret),
  ...
  const em = {
    ...
    findOneOrFail: jest.fn(async () => workspace),
    flush: jest.fn(),
    ...emOverrides,
  ...
  const service = new ApiKeyService(
    em,
    { setContext: jest.fn(), log: jest.fn() } as unknown as Logger,
    cryptService,
    { get: jest.fn(() => 'sk-') } as unknown as ConfigService,
