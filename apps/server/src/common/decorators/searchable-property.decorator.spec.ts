...
import { SearchableProperty } from './searchable-property.decorator';

const mockPropertyDecorator = jest.fn();

jest.mock('@mikro-orm/core', () => ({
  Property: jest.fn(() => mockPropertyDecorator),
}));
...
describe('SearchableProperty', () => {
  beforeEach(() => {
    jest.mocked(Property).mockClear();
  ...
  it('segments a source property with @node-rs/jieba search mode', () => {
    ...
    const propertyOptions = jest.mocked(Property).mock.calls[0][0] as
