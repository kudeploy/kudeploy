...
export class Migration20260601153226 extends RowLevelSecurityMigration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "workspace_member" ("id" bigserial primary key, "name" varchar(255) not null, "email" varchar(255) null, "searchable_name" tsvector null, "type" text check ("type" in ('USER', 'SERVICE_ACCOUNT')) not null default 'USER', "role" text check ("role" in ('OWNER', 'ADMIN', 'MEMBER')) not null default 'MEMBER', "permissions" text[] not null default '{}', "invited_by_id" bigint null, "invited_by_user_name" varchar(255) null, "invite_token" text null, "status" text check ("status" in ('ACTIVE', 'INVITING', 'INVITE_EXPIRED', 'DISABLED')) not null default 'ACTIVE', "invite_expires_at" timestamptz null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "user_id" bigint null, "workspace_id" bigint not null);`);
    this.addSql(`create index "workspace_member_searchable_name_index" on "public"."workspace_member" using gin("searchable_name");`);
    this.addSql(`create index "workspace_member_type_index" on "workspace_member" ("type");`);
    this.addSql(`create index "workspace_member_workspace_id_index" on "workspace_member" ("workspace_id");`);
    this.addSql(`create index "workspace_member_user_id_index" on "workspace_member" ("user_id");`);
    this.addSql(`create index "workspace_member_created_at_index" on "workspace_member" ("created_at");`);
    this.addSql(`create index "workspace_member_invite_token_index" on "workspace_member" ("invite_token");`);
    this.addSql(`create index "workspace_member_role_index" on "workspace_member" ("role");`);
    ...
    this.addSql(`create table "api_key" ("id" bigserial primary key, "name" varchar(255) not null, "key_id" varchar(255) not null, "key_prefix" varchar(255) not null, "encrypted_secret" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "last_used_at" timestamptz null, "expires_at" timestamptz null, "workspace_id" bigint not null, "member_id" bigint not null);`);
    this.addSql(`create index "api_key_created_at_index" on "api_key" ("created_at");`);
    this.addSql(`create index "api_key_member_id_index" on "api_key" ("member_id");`);
    this.addSql(`create index "api_key_workspace_id_index" on "api_key" ("workspace_id");`);
    this.addSql(`create index "api_key_key_prefix_index" on "api_key" ("key_prefix");`);
    this.addSql(`create index "api_key_key_id_index" on "api_key" ("key_id");`);
    this.addSql(`alter table "api_key" add constraint "api_key_key_id_unique" unique ("key_id");`);

    this.addSql(`create table "workspace_member_group" ("id" bigserial primary key, "name" varchar(255) not null, "searchable_name" tsvector null, "description" text null, "searchable_description" tsvector null, "permissions" text[] not null default '{}', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "workspace_id" bigint not null);`);
    this.addSql(`create index "workspace_member_group_searchable_description_index" on "public"."workspace_member_group" using gin("searchable_description");`);
    this.addSql(`create index "workspace_member_group_searchable_name_index" on "public"."workspace_member_group" using gin("searchable_name");`);
    this.addSql(`create index "workspace_member_group_created_at_index" on "workspace_member_group" ("created_at");`);
    this.addSql(`alter table "workspace_member_group" add constraint "workspace_member_group_name_workspace_id_unique" unique ("name", "workspace_id");`);

    this.addSql(`create table "workspace_member_group_member" ("id" bigserial primary key, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "workspace_id" bigint not null, "group_id" bigint not null, "member_id" bigint not null);`);
    this.addSql(`create index "workspace_member_group_member_created_at_index" on "workspace_member_group_member" ("created_at");`);
    this.addSql(`alter table "workspace_member_group_member" add constraint "workspace_member_group_member_group_id_member_id_unique" unique ("group_id", "member_id");`);
    ...
    this.addSql(`alter table "api_key" add constraint "api_key_workspace_id_foreign" foreign key ("workspace_id") references "workspace" ("id") on update cascade on delete cascade;`);
    this.addSql(`alter table "api_key" add constraint "api_key_member_id_foreign" foreign key ("member_id") references "workspace_member" ("id") on update cascade on delete cascade;`);

    this.addSql(`alter table "workspace_member_group" add constraint "workspace_member_group_workspace_id_foreign" foreign key ("workspace_id") references "workspace" ("id") on update cascade on delete cascade;`);
    ...
    this.addSql(`create policy user_select_policy on "public"."user" as permissive for select to authenticated using (true);`);
    this.addSql(`alter table "public"."user" enable row level security;`);
    ...
    this.addSql(`alter table "public"."workspace" enable row level security;`);
    this.addSql(`drop policy if exists soft_delete_select_policy on "public"."workspace";`);
    this.addSql(`create policy soft_delete_select_policy on "public"."workspace" as restrictive for select using ("deleted_at" is null);`);
    this.addSql(`alter table "public"."workspace" enable row level security;`);
    this.addSql(`drop policy if exists soft_delete_update_policy on "public"."workspace";`);
    this.addSql(`create policy soft_delete_update_policy on "public"."workspace" as restrictive for update using ("deleted_at" is null) with check (true);`);
    this.addSql(`alter table "public"."workspace" enable row level security;`);
    ...
    this.addSql(`drop policy if exists workspace_insert_policy on "public"."workspace";`);
    this.addSql(`create policy workspace_insert_policy on "public"."workspace" as permissive for insert to authenticated with check (true);`);
    this.addSql(`alter table "public"."workspace" enable row level security;`);
    this.addSql(`grant select on table "public"."workspace" to authenticated, anonymous;`);
    this.addSql(`drop policy if exists workspace_select_policy on "public"."workspace";`);
    this.addSql(`create policy workspace_select_policy on "public"."workspace" as permissive for select to authenticated, anonymous using (true);`);
    this.addSql(`alter table "public"."workspace" enable row level security;`);
...
\$\$;`);
    this.addSql(`revoke select, insert, update, delete on table "public"."api_key" from authenticated;`);
    ...
    this.addSql(`revoke select, update on table "public"."user" from authenticated;`);
    this.addSql(`revoke select, insert, update, delete on table "public"."workspace_member_group_member" from authenticated;`);
    this.addSql(`do \$\$ declare sequence_identifier text; begin for sequence_identifier in select pg_get_serial_sequence('"public"."workspace_member_group_member"', columns.column_name) from information_schema.columns where columns.table_schema = 'public' and columns.table_name = 'workspace_member_group_member' and pg_get_serial_sequence('"public"."workspace_member_group_member"', columns.column_name) is not null loop execute format('revoke usage, select on sequence %s from authenticated', sequence_identifier); end loop; end \$\$;`);
    this.addSql(`revoke select, insert, update, delete on table "public"."workspace_member_group" from authenticated, anonymous;`);
    this.addSql(`do \$\$ declare sequence_identifier text; begin for sequence_identifier in select pg_get_serial_sequence('"public"."workspace_member_group"', columns.column_name) from information_schema.columns where columns.table_schema = 'public' and columns.table_name = 'workspace_member_group' and pg_get_serial_sequence('"public"."workspace_member_group"', columns.column_name) is not null loop execute format('revoke usage, select on sequence %s from authenticated, anonymous', sequence_identifier); end loop; end \$\$;`);
    this.addSql(`revoke select, insert, update, delete on table "public"."workspace_member" from authenticated;`);
    this.addSql(`do \$\$ declare sequence_identifier text; begin for sequence_identifier in select pg_get_serial_sequence('"public"."workspace_member"', columns.column_name) from information_schema.columns where columns.table_schema = 'public' and columns.table_name = 'workspace_member' and pg_get_serial_sequence('"public"."workspace_member"', columns.column_name) is not null loop execute format('revoke usage, select on sequence %s from authenticated', sequence_identifier); end loop; end \$\$;`);
    this.addSql(`revoke select, insert, update, delete on table "public"."workspace_member" from authenticated;`);
    this.addSql(`do \$\$ declare sequence_identifier text; begin for sequence_identifier in select pg_get_serial_sequence('"public"."workspace_member"', columns.column_name) from information_schema.columns where columns.table_schema = 'public' and columns.table_name = 'workspace_member' and pg_get_serial_sequence('"public"."workspace_member"', columns.column_name) is not null loop execute format('revoke usage, select on sequence %s from authenticated', sequence_identifier); end loop; end \$\$;`);
    this.addSql(`revoke insert on table "public"."workspace" from authenticated;`);
    this.addSql(`do \$\$ declare sequence_identifier text; begin for sequence_identifier in select pg_get_serial_sequence('"public"."workspace"', columns.column_name) from information_schema.columns where columns.table_schema = 'public' and columns.table_name = 'workspace' and pg_get_serial_sequence('"public"."workspace"', columns.column_name) is not null loop execute format('revoke usage, select on sequence %s from authenticated', sequence_identifier); end loop; end \$\$;`);
