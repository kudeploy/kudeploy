...
const DbProbeSchema = new EntitySchema<DbProbe>({
  name: 'ServerE2eDbProbe',
  tableName: 'server_e2e_db_probe',
  properties: {
    id: {
      type: 'number',
      primary: true,
...
const databaseName = `server_e2e_${process.pid}_${Date.now()}`;
...
async function applyMigrations(orm: AdminOrm) {
  ...
  for (const file of migrationFiles) {
    const migrationModule = await import(
      `@/database/migrations/${file.replace(/\.ts$/, '')}`
    );
...
async function waitForServer(
  ...
  throw new Error(
    `Server did not become ready: ${String(lastError)}\n${getServerOutput()}`,
  );
}
