## 0.7.1 (2026-06-11)

### 🩹 Fixes

- **controller:** handle scale-only Service updates ([#21](https://github.com/kudeploy/kudeploy/pull/21))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.7.0 (2026-06-10)

### 🚀 Features

- limit replicas, consolidate settings, and add volumes ([caa0742](https://github.com/kudeploy/kudeploy/commit/caa0742))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.6.0 (2026-06-10)

### 🚀 Features

- **server:** prefix Kubernetes workspace labels ([#17](https://github.com/kudeploy/kudeploy/pull/17))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.5.1 (2026-06-10)

### 🩹 Fixes

- rename workspace label ([#14](https://github.com/kudeploy/kudeploy/pull/14))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.5.0 (2026-06-10)

### 🚀 Features

- add service volume bindings ([b149cbe](https://github.com/kudeploy/kudeploy/commit/b149cbe))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.4.0 (2026-06-10)

### 🚀 Features

- remove Project CRD backing ([#10](https://github.com/kudeploy/kudeploy/pull/10))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.3.3 (2026-06-09)

### 🩹 Fixes

- scale reserve deployments to zero ([6c258ba](https://github.com/kudeploy/kudeploy/commit/6c258ba))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.3.2 (2026-06-07)

This was a version bump only for controller to align it with other projects, there were no code changes.

## 0.3.1 (2026-06-07)

### 🩹 Fixes

- **controller:** propagate workspace labels from projects ([b056257](https://github.com/kudeploy/kudeploy/commit/b056257))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.2.1 (2026-06-06)

This was a version bump only for controller to align it with other projects, there were no code changes.

## 0.2.0 (2026-06-06)

This was a version bump only for controller to align it with other projects, there were no code changes.

## 0.1.2 (2026-06-05)

### 🩹 Fixes

- **controller:** test release trigger ([ab22418](https://github.com/kudeploy/kudeploy/commit/ab22418))

### ❤️ Thank You

- Xudong Huang @xudongcc

## 0.1.1 (2026-06-05)

### 🚀 Features

- **release:** migrate releases to nx ([b59ab85](https://github.com/kudeploy/kudeploy/commit/b59ab85))
- **charts:** add kudeploy Helm chart with CRDs and necessary templates ([fb15b1d](https://github.com/kudeploy/kudeploy/commit/fb15b1d))

### 🩹 Fixes

- **controller:** build image before release tagging ([74ccd81](https://github.com/kudeploy/kudeploy/commit/74ccd81))

### ❤️ Thank You

- Xudong Huang @xudongcc